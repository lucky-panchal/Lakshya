...
import { checkFile, checkURL, checkText } from "../api";
import SimilarityChart from "../components/SimilarityChart";
import ScoreRing from "../components/ScoreRing";
import { FileText, Link2, AlignLeft, Zap, Brain, Upload, AlertTriangle, CheckCircle, Info } from "lucide-react";
    ...
    <div className="max-w-5xl mx-auto py-10 px-4">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-5 gap-6">
        ...
        <motion.div initial={{ opacity: 0, x: -20 }} animate={{ opacity: 1, x: 0 }} className="lg:col-span-3 space-y-5">
          ...
          {/* Mode Selector */}
          <div className="glass rounded-2xl p-5">
            <p className="text-gray-400 text-sm font-medium mb-3">Detection Mode</p>
          ...
          <motion.button onClick={handleCheck} disabled={loading}
            ...
            className="w-full py-4 rounded-2xl font-semibold text-white text-sm transition-all duration-200 relative overflow-hidden"
                  ...
                  <div className="divide-y divide-white/5 max-h-64 overflow-y-auto">
                    {result.matches.map((m, i) => (
                      <motion.div key={i} initial={{ opacity: 0, x: 10 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: i * 0.05 }}
                        ...
                        <span className={`text-xs font-bold ml-2 shrink-0 ${m.similarity >= 70 ? "text-red-400" : m.similarity >= 40 ? "text-orange-400" : "text-green-400"}`}>
                          {m.similarity}%
                        </span>
                      </motion.div>
                    ))}
      ...
      {result && <SimilarityChart matches={result.matches} />}
    </div>
  );
}
